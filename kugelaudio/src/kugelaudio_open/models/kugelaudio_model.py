...
from .tokenizer import KugelAudioAcousticTokenizerModel, KugelAudioSemanticTokenizerModel
...
class KugelAudioModel(KugelAudioPreTrainedModel):
    def __init__(self, config):
        ...
        # PATCH: semantic_tokenizer added -  the decoder for inference (semantic features -> LM hidden space).
        ...
        # PATCH: semantic_connector added - Projects semantic features into the LM hidden space so they can be combined with acoustic features (combined_embed = acoustic_embed + semantic_embed).
    ...
    def strip_encoders(self):
        """Remove encoder weights from acoustic and semantic tokenizers to free VRAM.

        WARNING: After calling this, voice_prompt (raw audio cloning) will not work.
        Only call this if you are using pre-encoded voices exclusively (voice_cache).
        ...
        # PATCH: also strip the semantic encoder.
